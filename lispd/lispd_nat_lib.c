...
void nat_info_request(void)
{
    ...
    PATRICIA_WALK(AF4_database->head, node) {
        ...
        eid_prefix_length = locator_chain->eid_prefix_length;
        eid_address = locator_chain->eid_prefix;
        eid_address.afi = locator_chain->eid_prefix.afi;
...
#ifdef DEBUG
    syslog(LOG_DAEMON, "Sending ECM Map-Register packet");
#endif
...
{
    return (&(ctrl_iface->AF4_locators->head->db_entry->locator));
}
...
int compare_lisp_addresses(add1,add2)
...

{
    ...
    default:
        syslog(LOG_DAEMON, "compare_lisp_addresses: Unknown AFI %d",
               add1->afi);
        return (UNKNOWN);
    }
...
int add_rtr_as_default_in_map_cache(rtr_add)
...

{
    ...
    if (ERROR == install_map_cache_entry(&map_cache_entry)) {
        return (ERROR);
    }
...

{
    ...
    case AF_INET6:
        ...
        break;
    default:
        syslog(LOG_DAEMON, "inet_lisp_addr: unknown AFI (%d)", afi);
        break;
    }
...

{
    ...
    default:
        syslog(LOG_DAEMON, "inet2lispafi: unknown AFI (%d)", afi);
        return (0);
    }
}
...

{
    ...
    case AF_INET6:
        ...
        break;
    default:
        syslog(LOG_DAEMON, "inet_ntop_lisp: unknown AFI (%d)", afi);
        break;
    }
...

{
    ...
    case AF_INET6:
        ...
        break;
    default:
        syslog(LOG_DAEMON, "get_ntop_lisp_length: unknown AFI (%d)", afi);
        break;
    }
...

{
    ...
    default:
        syslog(LOG_DAEMON, "get_lisp_afi_len: unknown AFI (%d)", afi);
        return (ERROR);
    }
}
...
int complete_auth_fields(key_id,key_id_pos,key,packet,pckt_len,auth_data_pos)
 ...
 *
 */
int compute_sha1_hmac(key,packet,pckt_len,auth_data_pos,auth_data_len)
...

{
    ...
    *afi_ptr = htons(inet2lispafi(address->afi));
    ...
    if ((copy_addr(ptr, address, 0)) != get_addr_len(address->afi)) {
        syslog(LOG_DAEMON, "Error coping address with inet afi (%d)",
               address->afi);
        return (ERROR);
    }
...
lisp_addr_t extract_lisp_address(ptr)
...

{
    ...
    syslog(LOG_DAEMON, "Address lisp afi %d\n",
           inet2lispafi(address->afi));

    printf("Address inet afi %d\n", address->afi);
    syslog(LOG_DAEMON, "Address inet afi %d\n", address->afi);
...

{
    ...
    if ((name =
         (char *) malloc(get_ntop_lisp_length(address->afi))) == NULL) {
        syslog(LOG_DAEMON, "malloc (syslog with eid_name): %s",
               strerror(errno));
        return (ERROR);
    }
    ...
    syslog(LOG_DAEMON, " %s Address: %s Afi inet: %d\n", text, name,
           address->afi);
...
/*
 * Loop to free all the members of a lisp_addr_list_t
 */

void free_lisp_addr_list(list)
...

{
    ...
    unsigned int map_register_pkt_len;
    unsigned int ecm_map_register_len;



    map_register_pkt = build_map_register_pkt(locator_chain);
    ...
    if (ERROR == send_packet(ecm_map_register,
                             ecm_map_register_len,
                             outer_addr_from,
                             outer_port_from,
...
lisp_addr_t *select_best_rtr_from_rtr_list(rtr_rloc_list)
...
int send_packet(pkt_ptr, pkt_len, src_addr, src_port, dst_addr, dst_port)
...

{
    ...
    if ((s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
        syslog(LOG_DAEMON, "socket (send_packet): %s", strerror(errno));
        return (ERROR);
    }
    ...
    if (bind
        ...
        syslog(LOG_DAEMON, "bind (send_packet): %s", strerror(errno));
        close(s);
        return (ERROR);
    }
    ...
    if ((nbytes = sendto(s,
        ...
        syslog(LOG_DAEMON, "sendto (send_packet: %s )", strerror(errno));
        close(s);
        return (ERROR);
    }
