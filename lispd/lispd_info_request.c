...
int build_and_send_info_request(nonce,key_type,key,ttl,eid_mask_length,
                                eid_prefix,src_addr, src_port, dst_addr, dst_port)
...

{
    ...
    if (info_request_pkt == NULL) {
        syslog(LOG_DAEMON, "Couldn't build info request packet");
        return (ERROR);
    }
    ...
    if (ERROR == complete_auth_fields(key_type,
        ...
        syslog(LOG_DAEMON, "HMAC failed for info-request");
        return (ERROR);
    }


    if (ERROR == send_packet(info_request_pkt,
                             packet_len,
                             src_addr,
                             src_port,
                             dst_addr,
        ...
        syslog_with_address_name(LOG_DAEMON,
                                 "Couldn't send info-request for",
                                 eid_prefix);
        free(info_request_pkt);
        return (ERROR);
    }
