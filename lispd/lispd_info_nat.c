...
int process_info_nat_msg(packet,s,from,afi)
...

{
    ...
    switch (nat_pkt->rbit) {
    case NAT_NO_REPLY:
        syslog(LOG_DAEMON, "process_info_nat_msg: r-bit value not supported");
        return (ERROR);
...
lispd_pkt_info_nat_t *create_and_fill_info_nat_header(lisp_type,
                                                      reply,
                                                      nonce,
                                                      auth_data_len,
                                                      ttl,
...
unsigned int *header_len;
{
    ...
    if ((hdr = (lispd_pkt_info_nat_t *) malloc(hdr_len)) == NULL) {
        syslog(LOG_DAEMON, "malloc (header info-nat packet): %s", strerror(errno));
        return (NULL);
    }
    ...
    if ((copy_addr((void *) CO(eid_part,
                   ...
                   eid_prefix,
                   0)) != afi_len) {
        syslog_with_address_name(LOG_DAEMON, "Error coping eid address ",eid_prefix);
        free(hdr);
        return (NULL);
    }
...
int extract_info_nat_header(hdr,type,reply,nonce,key_id,auth_data_len,
                            auth_data,ttl,eid_mask_len,eid_prefix)
